...
ROOT_PATH = '/home/zhouxian/projects/experiments'
BACKGROUND_IDX = '0'
...
DEPTH_MAX = 80
RGB_TH = 10 # Minimum rgb difference after BS
...
BACKGROUND_RGB = cv2.imread(join(ROOT_PATH, args.target, 'videos/train/background_view0/00000.jpg')) 
...
def main(args):
    ...
    for file in os.listdir(join(datapath,  'videos', 'train')):
        if file[0:10] == 'background' or file.endswith('.mp4'):
...
def get_preprocessed_data(data_path, seqname, fr):
    ...
    rgb_path = join(data_path, 'videos', 'train',seqname, '{0:05d}.jpg'.format(fr))
...
def grabcut(datapath, seqname):
    ...
    save_path_rgb = join(datapath, 'rgb')
    ...
    for fr in np.arange(0, nFrames, 1):    
        ...
        cv2.imwrite(join(save_path_rgb, '{0}_{1:05d}.jpg'.format(seqname, fr)), img)
